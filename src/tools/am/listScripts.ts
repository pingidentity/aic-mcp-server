  ...
  async toolFunction({ realm }: { realm: string }) {
    try {
      ...
      url.searchParams.append(
        '_queryFilter',
        'context eq "AUTHENTICATION_TREE_DECISION_NODE" and evaluatorVersion eq "2.0"'
      );
      url.searchParams.append('_pageSize', '-1');
      url.searchParams.append(
        '_fields',
        '_id,name,description,language,context,evaluatorVersion,creationDate,lastModifiedDate'
      );
