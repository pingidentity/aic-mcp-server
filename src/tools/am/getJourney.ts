...
import { buildAMRealmUrl, AM_API_HEADERS, fetchNodeSchemas, fetchNodeConfigs, categorizeError } from '../../utils/amHelpers.js';
...
export const getJourneyTool = {
  ...
  async toolFunction({ realm, journeyName }: { realm: string; journeyName: string }) {
    try {
      ...
      const topLevelNodesWithType = topLevelNodeEntries.filter(n => n.nodeType) as Array<{ nodeId: string; nodeType: string }>;
      ...
      if (nestedNodeEntries.length > 0) {
        ...
        const newNodeTypes = nestedNodeTypes.filter(nt =>
          !schemaResults.some(sr => sr.nodeType === nt)
        );
      ...
      const enrichedJourney = {
        ...journey,
        nodeData: {
          schemas: schemaResults.reduce((acc, { nodeType, schema }) => {
            acc[nodeType] = schema;
            return acc;
          }, {} as Record<string, any>),
          configs: configResults.reduce((acc, { nodeId, config }) => {
            acc[nodeId] = config;
            return acc;
          }, {} as Record<string, any>),
        },
      ...
      return createToolResponse(`Failed to get journey "${journeyName}" in realm "${realm}" [${category}]: ${error.message}`);
    }
  },
};
